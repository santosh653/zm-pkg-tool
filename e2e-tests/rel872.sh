...
deploy()
{
   ...
   # zmb1-abc-libs
   mkdir -p build/stage/zmb1-abc-libs/opt/rr/lib

   cat > build/stage/zmb1-abc-libs/opt/rr/lib/abc-lib.sh <<EOM
ABC_LIB_VER="abc-lib-2"
EOM

   cat > build/stage/zmb1-abc-libs/opt/rr/lib/cmn-lib.sh <<EOM
CMN_LIB_VER="cmn-lib-2"
EOM

   ../../zm-pkg-tool/pkg-build.pl --out-type=binary --pkg-install-list='/opt/rr/' --pkg-name=zmb1-abc-libs --pkg-summary='its zmb-abc-libs' \
